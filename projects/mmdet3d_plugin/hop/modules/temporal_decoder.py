...
class TemporalDecoder(nn.Module):
    ...
    def __init__(self, embed_dims, bev_h, bev_w):
        ...
        self.features_fusion = nn.Conv2d(
            embed_dims + int(embed_dims / self.long_term_decoder.reduction),
            embed_dims,
            (3, 3),
            padding="same",
        )
    ...
    def forward(self, B_adj, B_rem):
        ...
        B_pred = torch.cat(
            [B_short, B_long], 2
        )  # concatenate short-term and long-term temporal information
...
class ShortTermTemporalDecoder(nn.Module):
    """
    Short-term temporal decoder module
    """
    ...
    def forward(self, B_adj):
        ...
        reference_points = reference_points.unsqueeze(2).repeat(
            1, 1, self.attn_cfg["num_levels"], 1
        )
        ...
        for V in B_adj:
            ...
            B_short += self.deform_attn(
                ...
                spatial_shapes=torch.tensor(
                    [[self.bev_h, self.bev_w]], device=Q_short.device
                ),
...
class LongTermTemporalDecoder(nn.Module):
    """
    Long-term temporal decoder module
    """
    ...
    def __init__(self, embed_dims, bev_h, bev_w, reduction=4):
        ...
        Args:
            ...
            bev_h (int): height of BEV feature maps
            bev_w (int): width of BEV feature maps
            reduction (int): reduction factor for the number of embedding dimensions
        """
        super().__init__()
        self.embed_dims = embed_dims
        self.bev_h = bev_h
        ...
        self.query_embed = nn.Embedding(
            self.bev_h * self.bev_w, int(self.embed_dims / self.reduction)
        )
        self.reference_points = nn.Linear(int(self.embed_dims / self.reduction), 2)
        self.reduction_layer = nn.Linear(
            self.embed_dims, int(self.embed_dims / self.reduction)
        )
    ...
    def forward(self, B_rem):
        ...
        B_long = torch.zeros(
            [bs, self.bev_h * self.bev_w, int(self.embed_dims / self.reduction)]
        ).to(device)
        ...
        for V in B_rem:
            ...
            B_long += self.deform_attn(
                ...
                reference_points=reference_points,
                spatial_shapes=torch.tensor(
                    [[self.bev_h, self.bev_w]], device=Q_long.device
                ),
