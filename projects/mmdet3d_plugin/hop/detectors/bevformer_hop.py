...
class BEVFormer_HoP(MVXTwoStageDetector):
    """
    BEVFormer_HoP: BEVFormer for 3D Object Detection with HoP.
    """
    ...
    def __init__(
        ...
        super(BEVFormer_HoP, self).__init__(
            pts_voxel_layer,
            pts_voxel_encoder,
            pts_middle_encoder,
            pts_fusion_layer,
        ...
        # Initialize HoP module:
        if self.hop_ckpts is not None:
            self.hop = HoP(
                hop_pred_idx=self.hop_pred_idx,
            ...
            state_dict = _load_checkpoint_with_prefix(
                prefix="hop", filename=self.hop_ckpts
            )
    ...
    def init_pretrained_weights(self):
        ...
        state_dict = _load_checkpoint_with_prefix(
            prefix="img_backbone", filename=self.pretrained_bevformer
        )
        load_state_dict(self.img_backbone, state_dict)

        state_dict = _load_checkpoint_with_prefix(
            prefix="img_neck", filename=self.pretrained_bevformer
        )
        load_state_dict(self.img_neck, state_dict)

        state_dict = _load_checkpoint_with_prefix(
            prefix="pts_bbox_head", filename=self.pretrained_bevformer
        )
    ...
    def forward_train(
        ...
        if self.bi_loss:
            ...
            prev_img = img[:, : -(1 + self.hop_pred_idx), ...]
            ...
            for key in losses_hop.keys():
                loss = (
                    self.hop_weight * losses_hop[key]
                    + (1 - self.hop_weight) * losses_pts[key]
                )
                losses[key] = loss
